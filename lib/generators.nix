...
${expr "" v}
  ...
  */
  ...
  mkShellArgument = {prefix ? "--", sep ? " ", disablePrefix ? null, ...}: k: v:
    let
      err = t: v: abort
  ...
  in attrs:
    libStr.concatStringsSep argSep (lib.concatLists (libAttr.mapAttrsToList (mkArgument') attrs));
}
